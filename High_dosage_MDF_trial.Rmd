...
library("readxl")
mtdata <- read_xlsx("/Users/shashankgupta/Desktop/ImprovAFish/ImprovAFish_4_percentage/MetaTranscriptomics/MetaTNMBUTrialAnalysis.Expression.Annotated.xlsx")
mtdata <- as.data.frame(mtdata)
rownames(mtdata) <- mtdata$GeneID
mtdata <- subset(mtdata, grepl("tryptophan", kegg_hit, ignore.case = TRUE))
mtdata$GeneID <- NULL
sample_info <- read.table("/Users/shashankgupta/Desktop/ImprovAFish/ImprovAFish_4_percentage/MetaTranscriptomics/sample_info.txt", header = T, row.names = 1, sep = "\t")

table(colnames(mtdata) %in% rownames(sample_info))
filtered_mtdata <- mtdata[, colnames(mtdata) %in% rownames(sample_info)]
table(colnames(sample_info) %in% rownames(filtered_mtdata))
table(colnames(filtered_mtdata) %in% rownames(sample_info))

sample_info<- sample_info[- grep("IFC_D3_tk11_HGmM07", sample_info$Original_name),]

ordered_filtered_mtdata <- filtered_mtdata[, rownames(sample_info)]
all(colnames(ordered_filtered_mtdata) == rownames(sample_info))
...
# Merge sample_info and ordered_filtered_mtdata based on sample names
merged_data <- merge(sample_info, as.data.frame(t(ordered_filtered_mtdata)), by = "row.names")

# Rename the merged_data column names for clarity
colnames(merged_data)[1] <- "SampleID"

# Convert the New_Diet variable to a factor for better plotting
merged_data$New_Diet <- as.factor(merged_data$New_Diet)

# Gather the data for ggplot
gathered_data <- tidyr::gather(merged_data, key = "OTU", value = "Abundance", -SampleID, -sampleType, -Original_name, -New_Diet)

# Create a boxplot using ggplot2
ggplot(gathered_data, aes(x = New_Diet, y = log(Abundance), fill = New_Diet)) +
  geom_boxplot() +
  labs(x = "New Diet", y = "Abundance") + theme_bw() 

```

```{r, eval=FALSE, echo=FALSE, results='hide'}
mtdata_raw <- read_xlsx("/Users/shashankgupta/Desktop/ImprovAFish/MetaTranscriptomics/MetaTDataDirdalTrialAnalysisAnnotated.xlsx")
mtdata <- as.data.frame(mtdata_raw)

rownames(mtdata) <- mtdata$GeneID
mtdata <- subset(mtdata, grepl("lactobacillus", GeneID, ignore.case = TRUE))
mtdata$GeneID <- NULL

selected_columns <- mtdata[, grep("T2|T3|cazy|kegg", colnames(mtdata))]

colnames_mtdata <- colnames(mtdata)
sample_info <- data.frame(SampleID = colnames_mtdata)
sample_info <- sample_info[c(1:151),]
sample_info_df <- data.frame(SampleID = sample_info)

# Split the SampleID column into Time, New_Diet, Tank, and Tissue
sample_info_df <- sample_info_df %>%
  separate(SampleID, into = c("Time", "New_Diet", "Tank", "Tissue"), sep = "_")
row.names(sample_info_df) <- sample_info
sample_info_df$Tank <- NULL
sample_info_df$Tissue <- NULL

table(colnames(mtdata) %in% rownames(sample_info_df))
filtered_mtdata <- mtdata[, colnames(mtdata) %in% rownames(sample_info_df)]
table(colnames(sample_info_df) %in% rownames(filtered_mtdata))
table(colnames(filtered_mtdata) %in% rownames(sample_info_df))
rows_to_remove <- colnames(sample_info_df) %in% rownames(filtered_mtdata)
sample_info_df <- sample_info_df[!rows_to_remove, ]

merged_data <- cbind(sample_info_df, as.data.frame(t(filtered_mtdata)))
merged_data$Tank <- NULL
library(tidyr)
merged_data_long <- gather(merged_data, Key, Value, -Time, -New_Diet)
merged_data_long$Value <- as.numeric(merged_data_long$Value)
# Create a boxplot using ggplot2
ggplot(merged_data_long, aes(x = New_Diet, y = log10(Value), fill = New_Diet)) +
  geom_boxplot() +
  labs(x = "New Diet", y = "Abundance") + theme_bw() + facet_wrap("Time")
